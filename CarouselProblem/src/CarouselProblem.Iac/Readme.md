...
Notes:
* `app` is an abbreviation for CarouselProblem, use the acronym of your choice and replace app with that (for example abc) using exact match - case sensitive find and replace in this file and AdStack.cs.
...
pulumi config set CarouselProblem.Iac:sql-server-app-db-admin-id
pulumi config set CarouselProblem.Iac:sql-server-app-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set CarouselProblem.Iac:default-email-from
pulumi config set CarouselProblem.Iac:email-server-host
pulumi config set CarouselProblem.Iac:email-server-port
pulumi config set CarouselProblem.Iac:email-server-userName
pulumi config set CarouselProblem.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set CarouselProblem.Iac:identity-certificate-password --secret
```
